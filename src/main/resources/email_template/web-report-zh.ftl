                            ...
                            >
                                <div style="margin-bottom:10px;">
                                    <el-radio-group v-model="serchData.status" size="small">
                                        <el-radio-button label="" @click.native="clickRadio($event, -1)">{{'全部（' +
                                            dataList.length+'）'}}
                                        </el-radio-button>
                                        <el-radio-button :label="0" @click.native="clickRadio($event, 0)">{{'成功（' +
                                            dataList.filter(item => item.status === 0).length+'）'}}
                                        </el-radio-button>
                                        <el-radio-button :label="1" @click.native="clickRadio($event, 1)">{{'失败（' +
                                            dataList.filter(item => item.status === 1).length+'）'}}
                                        </el-radio-button>
                                        <el-radio-button :label="2" @click.native="clickRadio($event, 2)">{{'跳过（' +
                        ...
                        <el-table
                            ...
                            <el-table-column width="100" align="center" label="状态">
                                <template slot-scope="scope">
                                    ...
                                    <el-tag v-if="scope.row.status === 1" size="small" type="danger">{{ "执行失败" }}
                                    </el-tag>
